...
try:
    ...
    raise ImportError("ERROR: This is a TensorFlow Lite model and requires \
                      TensorFlow Lite interpreter to be installed on this \
                      device. Please go to \
                      https://www.tensorflow.org/lite/guide/python and \
                      download the appropriate version for you device.")


class ImageClassificationModel():

    def __init__(self, signature):
        self.__model_path = "{}/{}".format(signature.model_path,
                                           signature.filename)
    ...
    def predict(self, image: Image.Image) -> PredictionResult:
        ...
        np_image = (np.float32(np_image) - self.__mean) / self.__std

        input_details = self.__tflite_predict_fn.get_input_details()
        output_details = self.__tflite_predict_fn.get_output_details()

        self.__tflite_predict_fn.set_tensor(input_details[0]['index'],
                                            np_image)
        self.__tflite_predict_fn.invoke()
        top_prediction_output = \
            self.__tflite_predict_fn.get_tensor(output_details[0]['index'])
        confidences_output = \
            self.__tflite_predict_fn.get_tensor(output_details[1]['index'])

        confidences = np.squeeze(confidences_output)
        top_prediction = top_prediction_output.item().decode('utf-8')

        sorted_labels, top_confidences = self.__sort_predictions(confidences,
                                                                 self.__labels)

        return PredictionResult(labels=list(zip(sorted_labels,
                                top_confidences)), prediction=top_prediction)
